...
public class TraineeRepositoryImplTest {
    ...
    void setUp() {
        ...
        traineeEntity.setDateOfBirth(new Date());
    ...
    void findByTraineeFromUsername_ShouldReturnTraineeEntity_WhenUsernameExists() {
        ...
        when(session.createQuery("from UserEntity where username = :username", TraineeEntity.class)).thenReturn(query);
    ...
    void findById_ShouldReturnTraineeEntity_WhenIdExists() {
        ...
        Long traineeId = 1L;
        Query<TraineeEntity> query = mock(Query.class);
        when(session.createQuery("from TraineeEntity where id = :traineeId", TraineeEntity.class)).thenReturn(query);
    ...
    @Test
    void save_ShouldPersistTraineeEntity() {
        // Act
        traineeRepository.save(traineeEntity);

        // Assert
        verify(session).persist(traineeEntity);
    }

    @Test
    void update_ShouldDetachTraineeEntity() {
        // Act
        traineeRepository.update(traineeEntity);

        // Assert
        verify(session).detach(traineeEntity);
    }
}
